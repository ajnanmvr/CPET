...
function Settings() {
  ...
  const getAllSettings = async () => {
    try {
      let { data } = await Axios.get("/configuration");
      setSettings(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleDelete = async (e, item) => {
    ...
    if (window.confirm("Do you want to remove this item")) {
      try {
        let res = await Axios.delete("/configuration/" + item);
        if (res.status === 200) {
          setConfig({ name: "", deadline: "", type: "" });
          getAllSettings();
          toast.success("settings deleted added", {
            autoClose: 3000,
  ...
  const getConfig = async () => {
    try {
      let { data } = await Axios.get(`/configuration/${id}`);
      setConfig(data);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div>
      ...
      <Link
        to={"/create-settings"}
        className="float-right m-4 py-3 px-4 bg-green-500 text-white font-bold hover:text-green-500 hover:bg-white border-green-400 border"
      >
        Add New
      </Link>
                  ...
                  <tr className="border-b" key={key}>
                    ...
                    <td className="px-5 py-5 bg-white text-sm">
                      <p className="font-bold">
                        {moment(setting?.deadline).format("ddd-mm-yyyy")}
                      </p>
...
function CreateSettings({ id, setConfig, config, setOpen, getConfig }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      let res = await Axios.patch("/configuration/" + id, config);
      if (res.status === 200) {
        setConfig({ name: "", deadline: "", type: "" });
